...
template "#{File.join(node[:rax_ruby_app][:user_home], 'rails_app', 'current', 'config', 'unicorn.rb')}" do
  ...
  mode 0660
  variables ({
    :working_directory => "#{File.join(node[:rax_ruby_app][:user_home], 'rails_app', 'current')}",
    :listen_port => node[:rax_ruby_app][:unicorn][:port],
    :worker_processes => node[:rax_ruby_app][:unicorn][:worker_processes],
    :pid => "#{File.join(node[:rax_ruby_app][:user_home], 'rails_app', 'current', 'unicorn.pid')}",
    :user => node[:rax_ruby_app][:user],
    :group => node[:rax_ruby_app][:group],
    :stderr => "#{File.join(node[:rax_ruby_app][:user_home], 'rails_app', 'current', 'unicorn_stderr.log')}",
    :stdout => "#{File.join(node[:rax_ruby_app][:user_home], 'rails_app', 'current', 'unicorn_stdout.log')}"
    })
end
